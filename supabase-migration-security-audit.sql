...
-- ===========================================================================
-- Fixes from a security audit. Run after supabase-migration-blocked-attempts.sql
-- (it's what the place_order below builds on).
--
-- 1. place_order trusted the browser's money. Line prices, subtotal, delivery
--    fee and total all came straight from the checkout payload, so anyone with
--    devtools could order a 3200 taka piece for 1. It now reads each price
--    from products.price, works the subtotal / delivery / total / advance out
--    itself, and raises PRICE_CHANGED or PRICE_MISMATCH if what was sent
--    doesn't match. A qty below 1 is refused too (INVALID_QTY).
--
-- 2. hero_slides' "Admin can view hero slides" read policy was `using (true)`
--    — it now checks is_admin(). The public homepage read goes through
--    "Anyone can view hero slides", which is untouched.
--
-- 3. A signed-in customer could join the wishlist under any email. They're now
--    held to the email on their account; visitors who aren't signed in are
--    unaffected.
...
-- ---------- 2. hero_slides: admin read is admin only ----------

drop policy if exists "Admin can view hero slides" on hero_slides;
create policy "Admin can view hero slides"
  on hero_slides for select
  to authenticated
  using (public.is_admin());

-- ---------- 3. wishlist: signed-in customers join with their own email ----------
